...
def main(args):
    ...
    for fold in range(args.folds):
        ...
        valid_samples = []
        with open(f'./data/{args.dataset}/valid_fold{fold}.csv', 'r') as file:
            for index, line in enumerate(file.readlines()):
                if index == 0:
                    pass
                else:
                    line = line.strip().split(',')
                    valid_samples.append((line[0], line[1], int(line[2]), int(line[3])))
        ...
        valid_dataset = MC_dataset(samples = valid_samples, index_map=index_map, page_subgraph=page_subgraph, rw_subgraph=rw_subgraph)
        valid_loader = DataLoader(dataset = valid_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=MC_collate)
        ...
        for epoch in range(args.epochs):
            ...
            valid_loss, valid_acc, valid_auroc, valid_f1_score, valid_precision, valid_recall, valid_ap = test_MC(model, criterion, valid_loader, Trans_emb_HG, Trans_emb_HY, device, args)
            msg = "epoch-%d, train_loss-%.4f, train_acc-%.4f, val_loss-%.4f, val_acc-%.4f, val_auroc-%.4f, val_f1_score-%.4f, val_prec-%.4f, val_rec-%.4f, val_ap-%.4f" % (epoch, train_loss, train_acc, valid_loss, valid_acc, valid_auroc, valid_f1_score, valid_precision, valid_recall, valid_ap)
            print(msg)
            with open(args.rst_file + f'fold{fold}/results.txt', 'a+') as fp:
                fp.write(msg + '\n')

        _, test_acc, test_auroc, test_f1_score, test_precision, test_recall, test_ap = test_MC(model, criterion, test_loader, Trans_emb_HG, Trans_emb_HY, device, args)
        test_msg = 'Test performance: test_acc-%.4f, test_auroc-%.4f, test_f1_score-%.4f, test_prec-%.4f, test_rec-%.4f, test_ap-%.4f' % (test_acc, test_auroc, test_f1_score, test_precision, test_recall, test_ap)
        print(test_msg)
